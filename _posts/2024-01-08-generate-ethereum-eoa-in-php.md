...
Адрес представляет собой 20-байтовое шестнадцатиричное число, которое используется для идентификации аккаунта в блокчейне Ethereum. Адрес - это уникальный идентификатор, который используется для отправки, получения и хранения Eth, токенов, а так же доступа к децентрализованным приложениям. Любой адрес состоит из строки буквенно-цифровых символов и обычно начинается с "0x", что указывает на его шестнадцатеричный формат.
...
Через конфиг указываем явно, что нам нужен ключ типа "эллиптическая кривавя". В качестве самой кривой указываем используемую в Ethereum secp256k1.
